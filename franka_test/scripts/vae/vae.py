...
class VAE(nn.Module):
    ...
    def __init__(self, img_dim, z_dim, s_dim, hidden_dim=[256,128],
        ...
        ylogvar_dim = np.prod(y_logvar_dim)
        encoder_input_dim  = int(input_dim + s_dim)
        encoder_output_dim = int(z_dim * 2)
        decoder_input_dim  = int(z_dim+s_dim)
        decoder_output_dim = int(input_dim + ylogvar_dim)
        ...
        self.ylogvar_dim = torch.tensor(ylogvar_dim)
        self.init=torch.tensor([False])
        self.learn_force = False
        self.device='cpu'
    ...
    def reset(self):
        ...
        if self.use_chunk_decode:
            self.build_chunk_decoder(self.num_chunks)
        if self.use_buffer:
            self.build_z_buffer()
    ...
    def forward(self, x, y, x_decode=torch.empty(0)):
        y_transform = self.img_encoder(y)
        enc_input = torch.cat([y_transform, x], dim=1) # y_transform
        ...
        y_pred, y_logvar = y_out[:,self.ylogvar_dim:], y_out[:,:self.ylogvar_dim]
        ...
        if not(x_decode.shape==torch.Size([0])):
            ...
            y_pred2, y_logvar2 = y_out2[:,self.ylogvar_dim:], y_out2[:,:self.ylogvar_dim]
            ...
            return img_pred, img_logvar , z_mu, z_logvar, z_samples, img_pred_decode, img_logvar_decode
        else: 
            return img_pred, img_logvar , z_mu, z_logvar, z_samples, x_decode, x_decode

    def decode_samples_only(self, samples, get_pred=False): 
    ...
    def update_dist(self, xr, y):
        out = self.forward(xr.to(device=self.device), y.to(device=self.device))
...
if __name__ == "__main__":
    ...
    print(z_out.shape)
    img_pred, img_logvar , z_mu, z_logvar, z_samples = vae(x,y)
